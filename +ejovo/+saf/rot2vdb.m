function vdbCoords = rot2vdb(safCoords)
...
safCoords = safCoords'; %transpose vdbCoords to facilitate rotation computation
[m, n] = size(safCoords);
vdbCoords = zeros(m, n);
for s = 1:n
    vdbCoords(:,s) = MtoVDB * safCoords(:,s); %#ok<MINV>
end

vdbCoords = vdbCoords'; %transpose safCoords back into an Nx3 matrix
